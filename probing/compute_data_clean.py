...
for data_path in data_paths:
    with open(data_path, "r") as f:
        ...
        if "data_samples" in json_data:
            data += json_data["data_samples"]
        else:
            data += json_data["samples"]

processed_samples = 0

instruction = "Please answer the following question with 'Yes' or 'No'."
for sample in tqdm(data):
    
    text = sample["context"]
    
    sample["qa_pairs"] = sample["qa_pairs"][:2]
    
    for qa in sample["qa_pairs"]:
        
        input_text = text + qa["question"]
