    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(text):
            ...
            for key, value in attention_records.items():
                sample_outputs[f"attention_layer{key[0]}_{key[1]}"] = value[i].numpy().tolist()
            for key, value in mlp_records.items():
                sample_outputs[f"MLP_layer{key[0]}_{key[1]}"] = value[i].numpy().tolist()
            sample_outputs["text"] = sample
            
            if (i+processed_samples) % 2 == 0:
                type = "clean"
            else:
                type = "corrupt"
